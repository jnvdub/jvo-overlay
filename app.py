...
def overlay_video():
    ...
    try:
        ...
        # Extract frames as JPEGs scaled to 9:16
        ...
        if extract.returncode != 0:
            raise Exception(f"Extract error: {extract.stderr[-300:]}")

        frames = sorted([f for f in os.listdir(frames_dir) if f.endswith(".jpg")])
        if not frames:
            raise Exception("No frames extracted from video")

        # Build PIL text overlay once
        ...
        for fname in frames:
            ...
            comp.save(os.path.join(comp_dir, fname), quality=88)

        # Build explicit file list for FFmpeg input
        comp_frames = sorted([f for f in os.listdir(comp_dir) if f.endswith(".jpg")])
        if not comp_frames:
            raise Exception("No composited frames found")
        ...
        with open(concat_path, "w") as cf:
            for fname in comp_frames:
                ...
                # Use double quotes around path (more compatible than single)
        ...
        if encode.returncode != 0:
            raise Exception(f"Encode error: {encode.stderr[-600:]}")
        ...
        for path in [input_path, frames_dir, comp_dir]:
