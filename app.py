...
def make_video(image_url, quote_text, reference):
    ...
    # Use app directory for frames — more reliable than /tmp on Railway
    ...
    for fi in range(OVERLAY_FRAMES):
        ...
        zw = int(1080 * zoom)
        zh = int(1080 * zoom)
        zoomed = base_img.resize((zw, zh), Image.LANCZOS)
        x_off = (zw - 1080) // 2
        y_off = (zh - 1080) // 2
        cropped = zoomed.crop((x_off, y_off, x_off + 1080, y_off + 1080))

        frame_canvas = Image.new("RGB", (VIDEO_W, VIDEO_H), (7, 4, 18))
        frame_canvas.paste(cropped, (0, 420))
        ...
        if fi >= ref_start:
            ...
            draw.text((rx, current_y + 18), ref_text, font=ref_font, fill=(*TEAL, int(255 * ar)))
        ...
        if fi >= handle_start:
            ...
            draw.text((bx, VIDEO_H - 75), brand, font=brand_font, fill=(255, 255, 255, int(200 * ar)))

        frame_rgba = frame_canvas.convert("RGBA")
        composited = Image.alpha_composite(frame_rgba, overlay).convert("RGB")
        frame_path = os.path.join(frame_dir, f"frame_{fi:04d}.jpg")
        composited.save(frame_path, format="JPEG", quality=85)

    # Verify frames were actually written
    written_frames = sorted(os.listdir(frame_dir))
    if len(written_frames) == 0:
        raise Exception(f"No frames written to {frame_dir}")

    # Use glob pattern input for FFmpeg — more reliable than printf pattern
    video_filename = f"{uid}.mp4"
    video_path = os.path.join(TEMP_DIR, video_filename)

    # Write a concat file listing every frame explicitly
    concat_path = os.path.join(TEMP_DIR, f"concat_{uid}.txt")
    with open(concat_path, "w") as f:
        for fname in written_frames:
            fpath = os.path.join(frame_dir, fname)
            duration = 1.0 / OVERLAY_FPS
            f.write(f"file '{fpath}'\n")
            f.write(f"duration {duration}\n")
        # Repeat last frame to avoid truncation
        last = os.path.join(frame_dir, written_frames[-1])
        f.write(f"file '{last}'\n")
