...
def build_video_overlay_filter(quote_text, reference, video_w=1080, video_h=1920):
    """Build FFmpeg drawtext filter string for video overlay."""
    import textwrap
    ...
    for line in lines:
        e = esc(line)
        filters.append(
            f"drawtext=fontfile='{QUOTE_FONT}':text='{e}':"
    ...
    ref_y = current_y + 20
    filters.append(
        f"drawtext=fontfile='{REF_FONT}':text='{ref_e}':"
    ...
    # Brand handle
    filters.append(
        f"drawtext=fontfile='{BRAND_FONT_F}':text='@only.jesusvibes':"
...
def overlay_video():
    ...
    try:
        ...
        filter_string = build_video_overlay_filter(quote_text, reference)
        cmd = [
