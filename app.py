...
def build_video_overlay_filter(quote_text, reference, video_w=1080, video_h=1920):
    """Build FFmpeg drawtext filter using textfiles to avoid escaping issues."""
    import textwrap

    QUOTE_FONT = QUOTE_FONT_PATH or FALLBACK_FONT_PATH
    REF_FONT = REF_FONT_PATH or FALLBACK_FONT_PATH
    BRAND_FONT_F = FALLBACK_FONT_PATH

    font_size = 58
    line_height = font_size + 14
    text_start_y = int(video_h * 0.52)
    panel_y = text_start_y - 40
    teal_line_y = text_start_y - 18

    # Wrap quote text
    wrapped = textwrap.fill(f'"{quote_text}"', width=32)
    lines = wrapped.split('\n')

    # Write text to temp files — avoids ALL escaping issues with special chars
    uid = uuid.uuid4().hex
    text_files = []

    for i, line in enumerate(lines):
        fpath = os.path.join(TEMP_DIR, f'line_{uid}_{i}.txt')
        with open(fpath, 'w') as f:
            f.write(line)
        text_files.append(fpath)

    ref_text = f"— {reference}, KJV"
    ref_file = os.path.join(TEMP_DIR, f'ref_{uid}.txt')
    with open(ref_file, 'w') as f:
        f.write(ref_text)

    brand_file = os.path.join(TEMP_DIR, f'brand_{uid}.txt')
    with open(brand_file, 'w') as f:
        f.write('@only.jesusvibes')

    filters = []

    # Dark panel
    filters.append(
        f"drawbox=x=0:y={panel_y}:w={video_w}:h={video_h - panel_y}:"
        f"color=0x070412@0.85:t=fill"
    )

    # Teal accent line
    filters.append(
        f"drawbox=x=60:y={teal_line_y}:w={video_w - 120}:h=3:"
        f"color=0x00C9A7@1.0:t=fill"
    )

    # Quote lines using textfile
    current_y = text_start_y
    for fpath in text_files:
        filters.append(
            f"drawtext=fontfile='{QUOTE_FONT}':textfile='{fpath}':"
            f"fontcolor=0x000000@0.7:fontsize={font_size}:"
            f"x=(w-text_w)/2+3:y={current_y + 3}"
        )
        filters.append(
            f"drawtext=fontfile='{QUOTE_FONT}':textfile='{fpath}':"
            f"fontcolor=0xFFF8E8:fontsize={font_size}:"
    ...
    ref_y = current_y + 20
    filters.append(
        f"drawtext=fontfile='{REF_FONT}':textfile='{ref_file}':"
        f"fontcolor=0x00C9A7:fontsize=44:"
        f"x=(w-text_w)/2:y={ref_y}"
    )

    # Brand handle
    filters.append(
        f"drawtext=fontfile='{BRAND_FONT_F}':textfile='{brand_file}':"
        f"fontcolor=0xFFFFFF@0.7:fontsize=30:"
        f"x=(w-text_w)/2:y={video_h - 80}"
    )

    # Return filter string AND list of temp files to clean up after FFmpeg runs
    return ','.join(filters), text_files + [ref_file, brand_file]
...
def overlay_video():
    ...
    try:
        ...
        video_w, video_h = 1080, 1920  # defaults
        try:
            import json as json_mod
            probe_data = json_mod.loads(probe.stdout)
            for stream in probe_data.get("streams", []):
                if stream.get("codec_type") == "video":
                    video_w = stream["width"]
                    video_h = stream["height"]
                    break
        except Exception:
            pass

        # If video is not 9:16, scale and pad to 1080x1920 first
        # then overlay text at 1080x1920 dimensions
        TARGET_W, TARGET_H = 1080, 1920
        if video_w != TARGET_W or video_h != TARGET_H:
            # Scale to fit within 1080x1920, pad with dark background
            scale_filter = (
                f"scale={TARGET_W}:{TARGET_H}:force_original_aspect_ratio=decrease,"
                f"pad={TARGET_W}:{TARGET_H}:(ow-iw)/2:(oh-ih)/2:color=0x070412"
            )
            filter_string, temp_text_files = build_video_overlay_filter(quote_text, reference, TARGET_W, TARGET_H)
            combined_filter = scale_filter + "," + filter_string
        else:
            filter_string, temp_text_files = build_video_overlay_filter(quote_text, reference, video_w, video_h)
        ...
        # Check output file actually has content
        ...
        return jsonify({
            ...
            "filename": filename,
            "ffmpeg_stderr_tail": result.stderr[-300:],
