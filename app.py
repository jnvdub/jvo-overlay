...
def make_video(image_url, quote_text, reference):
    ...
    - 150 composite JPEG frames at 10fps (not PNG, much smaller)
    - Ken Burns zoom done in Python per frame
    - FFmpeg just encodes the JPEG sequence — no complex filters
    """
    FPS = 24
    TOTAL_SECONDS = 15
    OVERLAY_FPS = 10
    ...
    # Generate composite frames directly (base + overlay merged in Python)
    ...
    for fi in range(OVERLAY_FRAMES):
        ...
        # Composite and save as JPEG (much smaller than PNG)
        frame_rgba = frame_canvas.convert("RGBA")
        composited = Image.alpha_composite(frame_rgba, overlay).convert("RGB")
        composited.save(os.path.join(frame_dir, f"frame_{fi:04d}.jpg"), format="JPEG", quality=85)

    # Encode with FFmpeg — simple JPEG sequence, no complex filters needed
