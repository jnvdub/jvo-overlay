...
def overlay_video():
    ...
    try:
        ...
        # Build PIL text overlay at target dimensions
        ...
        try:
            for stream in probe_data.get("streams", []):
                if stream.get("codec_type") == "video":
                    r = stream.get("r_frame_rate", "24/1").split("/")
                    fps = int(r[0]) // int(r[1]) if len(r) == 2 else 24
                    break
        except Exception:
            pass
        ...
        result = subprocess.run([
            "ffmpeg", "-y",
            "-framerate", str(fps),
            "-i", os.path.join(comp_dir, "frame_%04d.jpg"),
        ...
        if result.returncode != 0:
            raise Exception(f"FFmpeg encode error: {result.stderr[-400:]}")
