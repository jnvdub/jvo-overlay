...
def compose_image(image_url, quote_text, reference):
    ...
    # Total height: quote lines + gap + teal line + gap + ref + gap + brand
    total_quote_height = len(lines) * line_height
    total_block_height = total_quote_height + 20 + 3 + 16 + 48 + 16 + 30

    # Start quote at just below 50% mark
    quote_y = int(CANVAS_SIZE[1] * 0.52)

    # Draw dark gradient behind entire text block
    panel_top = quote_y - 30
    panel_bottom = CANVAS_SIZE[1]
